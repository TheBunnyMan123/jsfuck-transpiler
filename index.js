...
if (process.argv[process.argv.length - 1] == '-h') {
    console.log('Set last argument to "false" to disable eval-wrapping of final string')
    exit
}

var eval = !(process.argv[process.argv.length - 1] == 'false')
var str = ''
...
function string(str) {
    ...
    str.split('').forEach(element => {
        if (map[element] == undefined) {
            ...
            finalucode = ('0').repeat(4-ucode.length) + ucode
...
for (const key in map) {
    ...
    // console.log(key, eval(element))
}

transpiled = string(str)
